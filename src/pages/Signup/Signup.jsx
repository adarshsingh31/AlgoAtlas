...
const Signup = () => {
    ...
    script.onload = () => {
      if (window.google && window.google.accounts && window.google.accounts.id) {
        ...
        window.google.accounts.id.renderButton(
          ...
          { theme: 'filled_black', size: 'large', width: 336, shape: 'pill', text: 'signup_with' }
        );
      } else {
  ...
  const handleGoogleCredential = async (response) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Google sign-up failed. Please try again.");

      const data = await res.json();
      setStatus({ message: `Account ready for ${data.user?.email || "your Google account"}.`, type: "success" });
      setToken(data.token);
    } catch (err) {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(err.message || "Couldn't create your account. Try a different email.");
      }
